...
await ensureDirExists(CONFIG.datasetPath);
...
async function loadImage(imagePath) {
  try {
    ...
    return tfImage
      .resizeBilinear(CONFIG.imageSize)
      .toFloat()
      .div(tf.scalar(255));
  } catch (error) {
...
async function prepareDataset() {
  ...
  const labelMap = Object.fromEntries(
    CONFIG.predefinedCategories.map((cat) => [cat, []])
  );
  labelMap.other = [];
  ...
  for (const file of pngFiles) {
    ...
    if (!categoryFound) {
      labels.push(
        tf.oneHot(
          CONFIG.predefinedCategories.length,
          CONFIG.predefinedCategories.length + 1
        )
      );
      const subcategory = truncateName(sanitizedFilename.split("_")[0]);
