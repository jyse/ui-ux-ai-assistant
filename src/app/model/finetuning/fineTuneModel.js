...
async function loadPreprocessedData() {
  const dataPathHighContrast = path.join(
    __dirname,
    "..",
    "prepareDataset",
    "utilities",
  ...
  const PPDataHighContrast = JSON.parse(
    fs.readFileSync(dataPathHighContrast, "utf8")
  );
  const PPDataLowContrast = JSON.parse(
    fs.readFileSync(dataPathLowContrast, "utf8")
  );

  // Ensure data is flat/TypedArray before converting to tensors
  const xsHighContrast = tf.tensor4d(PPDataHighContrast.flat(), [
    PPDataHighContrast.length,
    224,
    224,
    3
  ]);
  const xsLowContrast = tf.tensor4d(PPDataLowContrast.flat(), [
    PPDataLowContrast.length,
    224,
    224,
    3
  ]);
  ...
  const ysHighContrast = tf.oneHot(
    tf.ones([PPDataHighContrast.length], "int32"),
    2
  );
  const ysLowContrast = tf.oneHot(
    tf.zeros([PPDataLowContrast.length], "int32"),
    2
  );
